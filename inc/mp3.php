...
class YoutubeToMP3 {
	...
	public static function get($url, $action = self::LINK) {
		...
		/*$itemInfoUrl = "http://www.youtube-mp3.org/a/itemInfo/?video_id={$videoId}&ac=www&t=grp&r={$currentTime}&s=14020";
        echo $itemInfoUrl;
        ...
        $r = $itemInfo['r'];
        $h2 = $itemInfo['h2'];
        $ts_create = $itemInfo['ts_create'];
		$cc = self::cc($sequence);
		*/
        $itemInfoUrl = "http://www.youtubeinmp3.com/download/?video=https://www.youtube.com/watch?v={$videoId}";
        $itemInfo = self::httpRequest($itemInfoUrl);
        $html = str_get_html($itemInfo);
        $find = $html->find('#download', 0)->href;
		$MP3URL = "http://www.youtubeinmp3.com/{$find}";
